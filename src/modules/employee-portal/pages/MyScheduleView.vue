  ...
  <div class="space-y-5 lg:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-1 rounded-lg border border-border bg-card p-1 shadow-sm">
        ...
        <div
          class="px-4 text-[11px] font-semibold  tracking-normal text-primary-text dark:text-primary-text min-w-35 text-center">
          {{ monthLabel }}
        </div>
    ...
    <Card v-else-if="scheduleQuery.isError.value"
      class="border-rose-200 bg-rose-50 shadow-none dark:border-rose-900/40 dark:bg-rose-950/20">
    ...
    <div v-else class="grid gap-6 lg:grid-cols-3">
      ...
      <div class="space-y-4">
        <Card class="border-primary/20 bg-primary/10 shadow-none">
          <CardHeader class="py-4">
            <CardTitle class="text-xs font-semibold text-primary">Chi tiết ngày {{
              selectedKey?.split('-').reverse().join('/') }}</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div class="grid gap-3">
                <div class="flex items-center gap-3 rounded-lg border border-primary/20 bg-card p-3">
                  ...
                  <div>
                    ...
                    <p class="text-sm font-semibold text-primary-text dark:text-primary-text">{{ selectedShift.start }}
                      — {{ selectedShift.end }}</p>
                  </div>
                </div>
              </div>
      ...
      <Card class="overflow-hidden border-border-standard shadow-none lg:col-span-2">
        ...
        <div class="grid grid-cols-7">
          <div v-for="day in calendarDays" :key="day.key" @click="selectDay(day)" :class="[
            ...
            <div v-if="day.schedule" class="mt-2">
              <Badge
                class="h-5 w-full justify-center rounded-md border border-primary/20 bg-primary/10 px-1.5 py-0 text-[10px] font-semibold text-primary">
                {{ day.schedule.short }}
              </Badge>
            </div>
            <div v-if="day.isWeekend && day.date && !day.schedule" class="mt-2">
              <Badge variant="secondary"
                class="h-5 w-full justify-center rounded-md border-none px-1.5 py-0 text-[10px] font-semibold opacity-60">
                OFF</Badge>
            </div>
          </div>
        </div>
