...
<template>
  <header
    class="sticky top-0 z-40 flex h-16 items-center gap-4 bg-indigo-600 px-4 shadow-lg shadow-indigo-900/20 dark:bg-indigo-950">
    ...
    <div class="flex flex-1 items-center gap-3 min-w-0">
      ...
      <nav class="hidden md:flex items-center gap-1 text-sm min-w-0">
        <template v-for="(crumb, idx) in breadcrumbs" :key="crumb.to">
          <span v-if="idx > 0" class="text-indigo-300/70 mx-1">/</span>
          <RouterLink v-if="idx < breadcrumbs.length - 1" :to="crumb.to"
      ...
      <div class="hidden lg:flex flex-1 max-w-xs ml-2">
        <div class="relative w-full">
          <Search class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/50" />
          <input v-model="searchQuery" type="text" placeholder="Tìm kiếm nhân viên, phòng ban..."
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <button
        ...
        <Bell class="h-5 w-5" />
        <span class="absolute right-1.5 top-1.5 flex h-2 w-2 items-center justify-center rounded-full bg-rose-500">
