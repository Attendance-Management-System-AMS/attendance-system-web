...
<template>
  <div class="space-y-6">
    <PageHeader :title="employee?.fullName || 'Chi tiết nhân viên'" description="Thông tin hồ sơ nhân viên">
      <template #actions>
        ...
        <button v-if="employee" @click="router.push(`/employees/${employee.id}/register-face`)"
        ...
        <button v-if="employee" @click="router.push(`/employees/${employee.id}/edit`)"
    ...
    <div v-if="employeeQuery.isLoading.value"
    ...
    <div v-else-if="employeeQuery.isError.value"
      ...
      <button class="mt-3 text-indigo-600 underline hover:text-indigo-800 dark:text-indigo-400"
    ...
    <div v-else-if="employee"
      ...
      <div class="border-b border-slate-100 px-6 py-4 dark:border-slate-800">
        ...
        <p class="mt-1 text-lg font-semibold text-slate-900 dark:text-white">{{ employee.fullName }}</p>
      </div>
      ...
      <div class="grid gap-0 sm:grid-cols-2">
        <div v-for="row in infoRows" :key="row.label"
          ...
          <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">{{ row.label }}</p>
