                    ...
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50/50 dark:border-slate-800">
                            <th
                                ...
                                viên</th>
                            <th
                                class="px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                Ngày
                                chuẩn</th>
                            <th
                                class="px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-slate-400">
                            ...
                            <th
                                ...
                                thêm (h)</th>
                            <th
                                class="px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-slate-400 min-w-35">
                                Tỷ lệ</th>
                        </tr>
                    </thead>
                        ...
                        <tr v-for="(row, idx) in rows" :key="row.id"
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div
                                        :class="['flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold', getAvatarColor(idx)]">
                                        {{ row.initials }}
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-slate-900 dark:text-white">{{ row.name }}</p>
                                        <p class="text-xs text-slate-400 font-mono">{{ row.empCode }} · {{ row.dept }}
                                        </p>
                                    </div>
                                </div>
                            </td>
                            <td
                                class="px-4 py-3 text-center text-sm font-mono font-bold text-slate-700 dark:text-slate-300">
                                {{
                                row.standard }}</td>
                            <td
                                class="px-4 py-3 text-center text-sm font-mono font-bold text-slate-900 dark:text-white">
                                {{
                                row.actual }}</td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    :class="['text-sm font-mono font-bold', row.leave > 0 ? 'text-amber-600' : 'text-slate-400']">
                                    {{ row.leave }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    :class="['text-sm font-mono font-bold', row.absent > 0 ? 'text-rose-600' : 'text-slate-400']">
                                    {{ row.absent }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    :class="['text-sm font-mono font-bold', row.overtime > 0 ? 'text-indigo-600' : 'text-slate-400']">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <div class="flex-1 h-1.5 rounded-full bg-slate-100 dark:bg-slate-800">
                                        <div :class="['h-full rounded-full transition-all', getRateBarColor(getAttendanceRate(row))]"
                                            :style="{ width: `${getAttendanceRate(row)}%` }"></div>
                                    </div>
                                    <span
                                        :class="['text-xs font-bold min-w-9 text-right', getRateColor(getAttendanceRate(row))]">
