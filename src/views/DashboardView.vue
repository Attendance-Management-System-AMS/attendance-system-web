          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <div class="flex items-center gap-2">
              <span class="flex h-2 w-2 rounded-full bg-emerald-500">
                <span class="absolute h-2 w-2 animate-ping rounded-full bg-emerald-400 opacity-75"></span>
              </span>
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-slate-100 bg-slate-50/50 dark:border-slate-800">
                  ...
                  <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-400">Ca làm</th>
                  <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-400">Giờ vào</th>
                  <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-400">Giờ ra</th>
                  <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-400">Trạng thái</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                <tr
                  v-for="row in attendanceData"
                  :key="row.name"
                ...
                >
                  ...
                  <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{{ row.shift }}</td>
                  <td class="px-4 py-3 font-mono text-sm text-slate-700 dark:text-slate-300">{{ row.checkIn }}</td>
                  <td class="px-4 py-3 font-mono text-sm text-slate-700 dark:text-slate-300">{{ row.checkOut }}</td>
                  <td class="px-4 py-3">
      ...
      <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div class="rounded-xl bg-slate-50 dark:bg-slate-800 px-4 py-5 text-center">
            <p class="text-3xl font-black tracking-tight text-slate-900 dark:text-white font-mono">{{ timeString }}</p>
        ...
        <!-- Weekly chart -->
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h3 class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-4">Tỷ lệ có mặt tuần này</h3>
          <div class="flex items-end gap-1.5 h-28">
            ...
            >
              <div class="relative flex w-full flex-col items-center justify-end h-20 rounded-t-sm overflow-hidden">
                <div class="w-full rounded-sm bg-slate-100 dark:bg-slate-800 absolute inset-0"></div>
                <div
                  class="absolute bottom-0 w-full rounded-sm bg-indigo-500 transition-all duration-500"
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div class="space-y-2">
            <div class="rounded-xl bg-amber-50 dark:bg-amber-950/50 border border-amber-100 dark:border-amber-900 px-3 py-2.5">
              <p class="text-xs font-medium text-amber-700 dark:text-amber-400">5 đơn xin nghỉ chờ phê duyệt</p>
            </div>
            <div class="rounded-xl bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-900 px-3 py-2.5">
              <p class="text-xs font-medium text-indigo-700 dark:text-indigo-400">Tỷ lệ đúng giờ tuần này: 96%</p>
            </div>
            <div class="rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 px-3 py-2.5">
              <p class="text-xs font-medium text-slate-600 dark:text-slate-300">3 nhân viên mới cần cập nhật hồ sơ</p>
            </div>
          </div>
        </div>
      </div>
