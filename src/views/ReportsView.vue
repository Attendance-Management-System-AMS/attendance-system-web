      ...
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-2.5">
            <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-950">
        ...
        <div class="flex items-end gap-2 h-36">
          ...
          >
            <div class="relative w-full flex flex-col justify-end h-28 rounded-t-sm overflow-hidden">
              ...
              ></div>
              <div
                class="absolute bottom-0 left-0 right-0 flex items-center justify-center pb-1"
              >
      ...
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="space-y-3.5">
          <div v-for="d in deptPerformance" :key="d.dept" class="flex items-center gap-3">
            <span class="w-24 text-xs font-medium text-slate-600 dark:text-slate-300 shrink-0">{{ d.dept }}</span>
            <div class="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800">
              <div
                :class="['h-full rounded-full transition-all duration-700', getRateColor(d.rate)]"
                :style="{ width: `${d.rate}%` }"
              ></div>
            </div>
            <span :class="['text-xs font-bold min-w-[36px] text-right', d.rate >= 95 ? 'text-emerald-600' : d.rate >= 90 ? 'text-indigo-600' : 'text-amber-600']">
              {{ d.rate }}%
            </span>
          </div>
        </div>
      ...
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="flex h-4 w-full overflow-hidden rounded-full mb-5">
          <div
            ...
            :key="s.label"
            :class="['h-full transition-all', s.color]"
        ...
        <div class="space-y-2.5">
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <span class="text-sm font-bold text-slate-900 dark:text-white">{{ s.count.toLocaleString() }}</span>
              <span class="text-xs font-bold text-slate-400 min-w-[32px] text-right">{{ s.percent }}%</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-3">
          <div v-for="r in lateReasons" :key="r.reason" class="space-y-1.5">
            <div class="flex items-center justify-between text-xs">
              ...
              <span class="font-bold text-slate-900 dark:text-white">{{ r.count }} lần ({{ r.percent }}%)</span>
            </div>
