...
const scheduleMap: Record<
  string,
  { name: string; start: string; end: string; color: string; short: string }
> = {
  '2026-04-01': {
    name: 'Ca hành chính',
    start: '08:00',
...
const selectedShift = computed(() =>
  selectedKey.value ? (scheduleMap[selectedKey.value] ?? null) : null,
)
...
<template>
  <div class="min-h-screen space-y-4 px-4 py-4 sm:px-6 sm:py-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div
        class="flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-2 py-1.5 dark:border-slate-800 dark:bg-slate-900"
      >
        <button
          @click="prevMonth"
          class="rounded-lg p-1 text-slate-400 hover:bg-slate-100 active:bg-slate-200 dark:hover:bg-slate-800"
        >
          <ChevronLeft class="h-4 w-4" />
        </button>
        <span
          class="min-w-[110px] text-center text-xs font-bold capitalize text-slate-700 dark:text-white"
          >{{ monthLabel }}</span
        >
        <button
          @click="nextMonth"
    ...
    <!-- Calendar Card -->
    <div
      class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden dark:border-slate-800 dark:bg-slate-900"
    >
      <!-- Day-of-week headers -->
      <div class="grid grid-cols-7 border-b border-slate-100 dark:border-slate-800">
        <div
          ...
          :key="d"
          class="py-2.5 text-center text-[10px] font-bold uppercase"
          :class="['T7', 'CN'].includes(d) ? 'text-rose-400' : 'text-slate-400'"
        >
          {{ d }}
        </div>
      </div>
      ...
      <div class="grid grid-cols-7">
        <div
          ...
          :class="[
            ...
            day.isWeekend
              ...
              : 'hover:bg-slate-50 dark:hover:bg-slate-800/50 active:bg-indigo-50',
        ...
        >
          <div v-if="day.date">
            ...
            <div
              v-if="day.holiday"
              class="mt-0.5 rounded bg-rose-100 px-1 py-0.5 text-[8px] sm:text-[9px] font-bold text-rose-600 dark:bg-rose-950/40 truncate"
            >
              {{ day.holiday }}
            </div>
            ...
            <div
              v-else-if="day.schedule"
              :class="[
                'mt-0.5 rounded px-1 py-0.5 text-[8px] sm:text-[9px] font-bold truncate',
                colorMap[day.schedule.color],
              ]"
            >
              <!-- Mobile: short code -->
    ...
    <div class="flex gap-3 overflow-x-auto pb-0.5 text-[11px] scrollbar-hide">
      <span class="flex shrink-0 items-center gap-1.5 text-slate-500"
        ><span class="h-2.5 w-2.5 rounded-sm bg-indigo-500"></span>HC (08:00-17:00)</span
      >
      <span class="flex shrink-0 items-center gap-1.5 text-slate-500"
        ><span class="h-2.5 w-2.5 rounded-sm bg-amber-500"></span>Sáng (07:00-12:00)</span
      >
      <span class="flex shrink-0 items-center gap-1.5 text-slate-500"
        ><span class="h-2.5 w-2.5 rounded-sm bg-violet-500"></span>Đêm (22:00-06:00)</span
      >
      <span class="flex shrink-0 items-center gap-1.5 text-slate-500"
        ><span class="h-2.5 w-2.5 rounded-sm bg-rose-400"></span>Nghỉ lễ</span
      >
    </div>
    ...
    <Transition name="slide-up">
      <div
        v-if="selectedKey"
        class="rounded-xl border border-indigo-200 bg-white shadow-sm dark:border-indigo-900 dark:bg-slate-900"
      >
        <div
        ...
        >
          ...
          <button @click="selectedKey = null" class="text-slate-400 hover:text-slate-600">
        ...
        <div v-else class="p-4 text-center">
          ...
          <RouterLink
            to="/my/requests"
            class="mt-2 inline-flex items-center gap-1 text-xs font-bold text-indigo-600 dark:text-indigo-400"
          >
    ...
    >
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center justify-between text-sm">
          <div class="flex items-center gap-2">
            <span class="h-2.5 w-2.5 rounded-sm bg-violet-500"></span
            ><span class="text-slate-600 dark:text-slate-400">Ca đêm</span>
          </div>
          <span class="font-bold text-violet-700 dark:text-violet-400">2 ngày</span>
        </div>
        <div
          class="flex items-center justify-between border-t border-slate-100 pt-2 text-sm dark:border-slate-800"
        >
          <div class="flex items-center gap-2">
            <CheckCircle2 class="h-3.5 w-3.5 text-emerald-500" /><span
